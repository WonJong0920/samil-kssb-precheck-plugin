...
## D83. Codex P0 runtime probe — Python alias/stub 실행 불가 확인, Node/npm.cmd 가용, PATH 밖 실행 가능
- **Date**: 2026-07-06
- **Context**: D82의 S0/S1/S2/S3 전략 분기 전 실제 Codex 세션 runtime surface 확인 필요. 이번 probe는 no-install evidence만 수행하고 다운로드·설치·runtime adoption은 금지.
- **Decision(evidence)**:
  ① `python`/`py`/`python3`은 PowerShell에서 WindowsApps alias/stub로 resolve되지만 실행 시 access failure. `where.exe python/py`는 no match, checked Python install directories도 no match → 현재 Codex 세션에서 Python은 사용 불가.
  ② Node는 system Node `v24.16.0`로 실행 가능. npm은 bare PowerShell `npm`이 `npm.ps1` execution policy에 막히므로 `npm.cmd`(`11.13.0`) 또는 explicit executable 사용 필요.
  ③ `<TEMP>` repo-outside script와 fake tool-cache script를 absolute path로 실행 성공 후 삭제 → PATH 밖 executable/script 호출 자체는 가능. 단 실제 downloaded runtime binary 허용은 별도 승인 probe 필요.
  ④ portable Node/Python download, Kordoc reinstall, npm install, OCR/rasterizer/tesseract, tool-cache runtime adoption은 수행하지 않음. repo contamination scan clean.
- **Consequences**: S0는 real non-WindowsApps Python absolute path가 제공될 때만 가능. S1(Node migration)는 runner/test reviewability 측면에서 즉시성이 높다. S2 portable Python은 별도 download/hash/adoption 승인 gate 필요. S3 staged hybrid를 P0 이후 분기 프레임으로 유지. 2N-5는 Codex-like 환경 기준인지 Claude/user shell 기준인지 사용자/ChatGPT 결정이 필요하다.
- **Status**: evidence 문서 + status/decision만. 코드/테스트/schema/package/runner/provider 무변경, 설치·다운로드·실행 adoption 없음.
- **Related Files**: `docs/samples/codex_runtime_probe_evidence_p0.md`, `docs/planning/cycle2n_4c_runtime_strategy_python_reduction_plan.md`, `docs/reviews/codex_cycle2n_4c_runtime_strategy_plan_review.md`.

## D84. 2N-4D runner Node port — S1 1단계 구현(CLI 계약 동일·aux 제외·Python reference 유지), 2N-5 unblock 아님
- **Date**: 2026-07-06
- **Context**: 2N-4C 계획 리뷰 **PASS** + **P0 probe(D83)** 후 사용자/조율 판단 — S3 유지하되 1순위=S1 runner Node
  migration(2순위 S2 portable Python evidence gate·core 이식 보류·2N-5는 Codex-like 기준 Python blocker로 보류).
  사용자 지시로 P0-B probe 프롬프트 작성(병행 — Codex 측 전달) + 2N-4D 착수. Claude 사전 보완 3건 반영:
  2N-4D≠2N-5 unblock 명시·aux 제외 명시·no-overclaim 완료 표현.
- **Decision(구현)**:
  ① **`src/intake/runners/hwp_assisted_runner.cjs`**(Node 내장 모듈만·source-only·core 미참조): Python runner와 **CLI 계약
  동일** — 플래그(`--check/--approve-install/--approve-run/--evidence-mode/--out-dir/--tool-cache`)·exit code(0/2/3/4/5/6/7)·
  한국어 승인 문구(U5 — 승인 대화만 provider 명시, 완료 안내 미노출)·tool-cache 레이아웃·pin(kordoc@3.13.0+
  pdfjs-dist@4.10.38, `--omit=optional`)·prep_egress_log/approvals/run_log(key 정렬 JSON, UTC timestamp)·provenance
  (`no_egress_verified=true`는 [NETHOOK-SUMMARY] 실관측+egressAttempts=0에만, evidence 모드 미관측=RunnerError) —
  nethook.cjs 무변경 재사용, 무승인 설치/실행 금지 게이트 동일.
  ② **v1 의도적 차이(README 문서화)**: aux_signals 미생성(Node 내장에 zip/XML parser 부재 — Python aux_structure_scanner
  전용, HWPX/DOCX는 건너뜀 안내·기존 설계상 aux 부재 허용과 정합), node 실행 파일=process.execPath 자기 보장(npm 부재만
  exit 4), npm=PATH+PATHEXT 해석(Windows npm.cmd 자연 선택·npm.ps1 배제 — P0/AVR-04 계승).
  ③ **테스트 `tests/test_hwp_assisted_runner_node.test.cjs` 27/27**(node:test 내장 러너 — 외부 의존성 0·repo package.json
  불필요·**Codex 세션에서 직접 실행 가능**(P0에서 Node 가용 실증 — 검증 공백 축소)): 게이트(무승인 exec 0)/승인 분리(U3)/
  builder pin·global·npx 부재/provenance 4상태/한국어·공백·괄호 파일명/which PATHEXT(npm.cmd 선택·ps1 배제)/로그 형식/
  core 미참조·`node:` 내장 require만/**Python 소스 텍스트 대조 parity**(KORDOC·PDFJS pin, exit code 5종, egressAttempts
  정규식, `.intake.json` 규약, 확장자 집합 — Python 실행 없이 검증). 라이브 게이트: 무인자 rc=2·무승인 rc=5(한국어 문구)·
  check rc=0·side-effect 0(tool-cache 미생성).
  ④ **하지 않은 것**: 실 Kordoc 실행 parity evidence(이번 지시에 실행 승인 없음 — follow-up, 2N-2 mock 패턴 준수),
  portable Node/Python 다운로드, Python runner 변경/제거(reference 유지 — 최종 처리 별도 결정), core/ingest/validator/
  renderer/delivery 변경, OCR 계열.
- **Consequences**: **다음 = Codex 2N-4D review**(node:test 직접 실행 가능). 병행 = P0-B probe(실제 Python 절대 경로 —
  성공 시 Codex가 Python 회귀 7종도 직접 실행 가능). **2N-4D는 2N-5를 unblock하지 않음** — E2E의 ingest/core는 여전히
  Python. 2N-5 unblock 경로(S2 evidence gate vs core 이식 vs 환경 재정의)는 리뷰 대기 중 사용자/ChatGPT 결정 필요.
  정직한 상태 표현: "assisted 산출물 생성까지 Node 단일화 / ingest·보고서 생성은 Python 필요".
- **Status**: 변경 = runner .cjs(신규)·node 테스트(신규)·runners README·intake README(2N-4B PASS 문구 최신화 겸사)·
  status/decision. Python runner·core·schema/validator/renderer/delivery/manifest/package 무변경. 설치·다운로드·
  실 provider 실행 없음(mock·라이브 게이트 확인만), tool-cache 미생성.
- **Related Files**: `src/intake/runners/hwp_assisted_runner.cjs`, `tests/test_hwp_assisted_runner_node.test.cjs`,
  `src/intake/runners/README.md`, `src/intake/README.md`, `docs/planning/cycle2n_4c_runtime_strategy_python_reduction_plan.md`,
  `docs/samples/codex_runtime_probe_evidence_p0.md`, `docs/reviews/codex_cycle2n_4c_runtime_strategy_plan_review.md`.

## D85. 2N-4D-A Node runner 실패 처리 보정 — evidence 실패=통제된 exit 7(stack/경로 미노출), check 표시=resolved npm
- **Date**: 2026-07-06
- **Context**: Codex 2N-4D **CONDITIONAL PASS** — C2N4D-MAJ-01(evidence 모드 provenance 실패가 uncaught RunnerError로
  CLI에 새어 stack trace·로컬 코드 경로 노출 + 문서화된 7이 아닌 exit 1 — 프로그래매틱 테스트는 throw만 검증하고 CLI 실행
  형태는 미검증), C2N4D-MIN-01(check 모드 설치 명령이 bare `npm install` 표시 — 실제 실행은 npm.cmd지만 사용자 복붙 시
  P0/AVR-04 정책과 충돌 가능). narrow patch — runtime 전략·portable·Kordoc 실행·core/ingest/nethook/Python runner 불가침.
- **Decision(구현)**:
  ① **MAJ-01**: `main()` 내 run provenance 단계에서 RunnerError를 catch — **정직한 provenance를 먼저 기록**
  (`buildRunProvenance(output, false)` 재계산 → run_log에 no_egress_verified=false 항목) 후 RunnerError의 한국어 메시지 +
  기존 실패 안내 문구를 출력하고 **EXIT_RUN_FAILED(7)** 반환. CLI 경계(require.main)에도 방어 catch(RunnerError→문구+7,
  그 외 예외는 유지). `buildRunProvenance()`의 throwing API 자체는 불변(프로그래매틱 사용·기존 4상태 테스트 유지).
  ② **MIN-01**: check 모드 설치 명령 표시를 `buildInstallCommand(toolCache, node.npm)`으로 — 실행과 동일한 resolved
  npm 경로(Windows: npm.cmd) 표시, bare npm 표시 금지.
  ③ **테스트 27→29/29**(node:test): 기존 evidence 테스트를 통제된 실패 계약으로 갱신(rc=7·thrown null·문구·정직 로그) +
  **CLI subprocess 실검증 신규** — 실제 `node hwp_assisted_runner.cjs <doc> --approve-run --evidence-mode` 실행(fake
  repo-외부 tool-cache의 fake cli.js가 차단되는 원격 DNS 시도 — nethook이 원본 호출 전 throw라 **외부 트래픽 0**) →
  exit=7·stdout/stderr에 "RunnerError"/stack("at ")/`hwp_assisted_runner.cjs:<line>`/repo 경로 미노출·run_log에
  hook_observed=true·egress≥1·verified=false 확인(Codex 리뷰의 재현 방식과 동일 계열) + check 모드 npm.cmd 표시/bare npm
  부재 테스트.
  ④ **기록된 후속 항목**: Python runner도 evidence 실패 시 traceback exit 1(동일 특성) + check 표시 bare npm — 이번
  사이클 변경 금지 범위라 미보정(후속 결정 항목).
- **Consequences**: **다음 = Codex 2N-4D-A narrow 재리뷰.** 재리뷰 PASS 시 Node runner를 assisted 실행 evidence 수단으로
  사용 가능(리뷰 권고). 2N-5 unblock 아님(불변 — ingest/core Python blocker는 별도 트랙: S2 evidence gate 등 결정 대기).
- **Status**: 변경 = runner .cjs·node 테스트·runners README·status/decision. nethook/Python runner/core/ingest/schema/
  validator/renderer/delivery/manifest/package 무변경. 설치·다운로드·실 Kordoc 실행 없음(fake tool-cache·차단 DNS 시도는
...
## D87. 2N-4F portable Node B안 mock 구현 — 승인 게이트 bootstrap + 이중 hash fail-fast + 탐지 계층, 실 다운로드 0
- **Date**: 2026-07-06
- **Context**: 2N-4E 계획 리뷰 **PASS**(required fixes 없음, OBS-01~03은 이번 테스트 범위·2N-4G 요건으로 반영) 후 착수.
  **사용자 결정 5건 승인 기록**(C2N4E-OBS-01 대응): ①B안 구현 경로 채택 ②pin=v24 LTS 계열 ③PowerShell
  `-ExecutionPolicy Bypass` 1회 사용을 승인 UX에 포함 ④submission.zip에 portable Node 바이너리 미포함 ⑤Python runner
  미확장. 이번 사이클은 **mock 전용** — 실 다운로드·nodejs.org 접속·npm install·Kordoc 재설치·tool-cache 실 runtime
  설치 전부 금지 준수.
- **Decision(구현)**:
  ① **`prepare_portable_node.ps1`**(신규 — Windows 내장 PowerShell만, source-only): 무승인 호출=승인 안내만·파일 생성
  0(exit 5 — 승인 문구에 대상/출처(nodejs.org/dist 고정)/용량/위치/이중 검증/시스템 무변경/Bypass 1회/거부 fallback 전부
  고지). **이중 SHA-256 검증**: (1차) repo-pinned 기대값 + (2차) SHASUMS256.txt 해당 행 교차 — 불일치·파싱 실패는 부분
  파일 정리 후 중단(해제 금지), 해제 후 `node.exe --version`=pin 자가 확인 + npm.cmd 동봉 확인(실패 시 배치 정리).
  approvals.json `runtime` marker + prep_egress_log(provider=nodejs-portable, key 정렬, **BOM 없는 UTF-8** — Node
  JSON.parse 호환). **`$PINNED_ZIP_SHA256_CONST=""`(미기록) + 실(https) 출처 → 네트워크·파일 생성 전 fail-closed 거부**
  (기대값 기록은 2N-4G evidence 사이클). `-SourceRoot` 로컬 디렉터리 지정으로 테스트 fixture 검증(출처가 로그에 남음).
  pin 후보 `24.16.0`(Gate D·2N-4·P0 실측 major — 최종 확정 2N-4G). 종료 코드 0/5/7.
  **인코딩 실증**: PS5.1은 BOM 없는 UTF-8 .ps1을 CP949로 읽어 파싱 실패 — **ps1은 UTF-8 BOM 필수**(2M-3 전례, README 기록).
  ② **runner 탐지 계층**(`hwp_assisted_runner.cjs`): `PORTABLE_NODE_VERSION` 상수 + `portableNodeDir/Paths()` +
  `detectNode(which, toolCache)` = **시스템(둘 다) → portable(node.exe+npm.cmd 절대 경로 — 불완전 시 혼용 금지 missing) →
  부재**(source 마커). 부재 안내(exit 4)에 B안 승인 절차(bootstrap 명령) 표시. exit/플래그/승인 게이트/nethook/
  provenance/Kordoc U1 계약 **불변**(2N-4D-A PASS 상태 — 기존 테스트 무수정 유지로 증명).
  ③ **테스트**: 신규 `tests/test_portable_node_bootstrap.test.cjs` **9/9** — 무승인 게이트(파일 0)·fail-closed(실 URL+pin
  미기록, tool-cache 미생성으로 선행성 증명)·성공 경로(**실 node.exe를 테스트 시점에 fixture zip으로 동봉** — 바이너리
  커밋 0·자가 버전 확인 실통과·marker/로그 검증)·pinned 불일치·SHASUMS 불일치·파싱 실패·손상 zip(hash 통과+해제 실패)·
  다운로드 실패·가짜 exe 버전 확인 실패+배치 정리 — **전부 로컬 fixture(실 네트워크 0)**. runner 스위트 29→**35/35**
  (탐지 우선순위·portable run 명령 절대 경로·B안 안내·ps1↔cjs pin parity·`24.` 계열 고정·hash 상수 미기록 확인).
- **Consequences**: **다음 = Codex 2N-4F review** → (가칭)**2N-4G 승인 실측 evidence**(실 다운로드 1회 — pin·기대 hash
  최초 관측 기록, 고의 불일치 실증, portable node에서 runner/nethook 관측, portable npm.cmd로 Kordoc 설치 성립 —
  C2N4E-OBS-03). B안 mock 완료 ≠ 채택 완료·2N-5 unblock 아님(ingest/core Python 트랙 별도) — no-overclaim 유지.
- **Status**: 변경 = ps1(신규)·runner .cjs(탐지 계층)·테스트 2파일·runners README·status/decision. Python runner/core/
  ingest/nethook/schema/manifest/package 무변경. 실 다운로드·설치·submission.zip 없음, repo에 바이너리/zip 커밋 없음.
- **Related Files**: `src/intake/runners/prepare_portable_node.ps1`, `src/intake/runners/hwp_assisted_runner.cjs`,
  `tests/test_portable_node_bootstrap.test.cjs`, `tests/test_hwp_assisted_runner_node.test.cjs`,
  `src/intake/runners/README.md`, `docs/planning/cycle2n_4e_portable_node_b_plan.md`,
  `docs/reviews/codex_cycle2n_4e_portable_node_b_plan_review.md`.

## D88. 2N-4F-A portable Node safety patch — 버전 실측 일치 필수 + 원격 출처 공식 고정·override 거부
- **Date**: 2026-07-06
- **Context**: Codex 2N-4F **CONDITIONAL PASS** — C2N4F-MAJ-01(portable 탐지가 node.exe/npm.cmd 존재만으로 인정,
  `--version` 대조 없음 → 손상/드리프트 재사용 위험), C2N4F-MAJ-02(임의 `https?://` SourceRoot를 원격으로 수용 +
  원격에서 `-PinnedZipSha256` override로 repo-pinned provenance 우회 가능). 2N-4G 실측 전 필수 안전 조건. narrow patch —
  실 다운로드·nodejs.org 접속·npm install·Kordoc 재설치·core/ingest/nethook/Python runner 불가침 준수.
- **Decision(구현)**:
  ① **MAJ-01**: `portableNodeVersionProbe(nodeExe)` 신규(spawnSync `--version`, encoding utf8, timeout 10s,
  실패/비정상=null) — `portableNodePaths()`가 파일 존재 + **실측 버전 === `v${PORTABLE_NODE_VERSION}`** 일 때만 유효
  반환(probe 예외도 null 처리). `detectNode(which, toolCache, probeFn)` 주입식(main은 opts.probeFn || 실체 probe).
  버전 명령 실패·timeout·불일치·파일 누락 전부 missing/corrupt → 시스템 Node 없으면 기존 승인 안내(exit 4)/A안 수렴.
  ② **MAJ-02**: ps1 출처 게이트(승인 후·네트워크/파일 생성 전) — 원격은 `$officialRoot = https://nodejs.org/dist/v<pin>`
  **정확 일치(TrimEnd('/') 비교)만 허용**, 비공식 미러/임의 URL은 fail-fast(exit 7·한국어 안내). **원격 경로에서는
  `-PinnedZipSha256` 지정 자체를 거부**(repo-pinned 상수만 유효 — 상수 미기록이므로 실 다운로드는 계속 fail-closed).
  로컬 디렉터리 SourceRoot는 테스트 fixture 전용 유지(override 허용·출처 로그 기록).
  ③ **테스트**: runner 35→**39/39** — 버전 일치 수용/불일치→missing/명령 실패·예외→missing/실체 probe(실제 node.exe=
  process.version, 가짜 exe=null)/portable run 흐름(버전 일치)·불일치 시 exit 4 수렴(exec 0). bootstrap 9→**11/11** —
  비공식 원격 fail-fast·공식 원격+override 거부(둘 다 tool-cache 미생성으로 게이트 선행성 증명). 로컬 fixture 성공
  경로·기존 실패 매트릭스 전부 유지. **실 네트워크 0**(원격 케이스는 게이트가 네트워크 호출 전에 거부).
- **Consequences**: **다음 = Codex 2N-4F-A narrow re-review.** 2N-4G 실측 evidence는 re-review PASS 후 판단
  (pin·기대 hash 최초 관측 기록은 그때 수행). B안 채택 완료·2N-5 unblock 아님(불변).
- **Status**: 변경 = runner .cjs·ps1·테스트 2파일·runners README·status/decision. Python runner/core/ingest/nethook/
  schema/manifest/package 무변경. 실 다운로드·설치·바이너리 커밋 없음.
- **Related Files**: `src/intake/runners/hwp_assisted_runner.cjs`, `src/intake/runners/prepare_portable_node.ps1`,
  `tests/test_hwp_assisted_runner_node.test.cjs`, `tests/test_portable_node_bootstrap.test.cjs`,
  `src/intake/runners/README.md`, `docs/reviews/codex_cycle2n_4f_portable_node_mock_implementation_review.md`.

## D89. 2N-4G portable Node real-download evidence — 공식 SHASUMS 교차검증 + repo-pinned hash 기록
- **Date**: 2026-07-06
- **Context**: 2N-4F-A narrow re-review PASS 후, 사용자 승인 범위 안에서 공식 `nodejs.org/dist/v24.16.0/`
  접근·Windows portable Node zip 1회 다운로드·SHASUMS256.txt 확인·repo-pinned hash 기록·bootstrap 실측 evidence를 Codex가 수행.
  이번 항목은 evidence 기록이며 Portable Node B 채택 완료, 2N-5 unblock, L2 complete, OCR support, provider finalization 선언이 아님.
- **Decision(evidence)**:
  ① **공식 source/hash**: `SHASUMS256.txt`의 `node-v24.16.0-win-x64.zip` row와 실제 zip SHA-256이
  `edaca9bd58ec8e92037dac4e877d52f6b8f430b81c18b57e264b4e2fb111cd56`로 일치. 이 값을
  `prepare_portable_node.ps1`의 `$PINNED_ZIP_SHA256_CONST`에 기록.
  ② **공식 remote bootstrap**: `prepare_portable_node.ps1 -ApproveRuntime -PinVersion 24.16.0` exit 0,
  repo 밖 `<TOOL_CACHE>\node@v24.16.0-win-x64` 배치, `node.exe --version=v24.16.0`, `npm.cmd --version=11.13.0`,
  prep log `started→ok`, approval marker 존재 확인.
  ③ **고의 실패 경로**: local fixture + 잘못된 expected SHA-256은 exit 7 `failed pinned-hash-mismatch`,
  배치 폴더 미생성으로 fail-fast·extract 금지 확인.
  ④ **runner/nethook**: system Node/npm을 숨긴 detection에서 `source=portable` 확인. portable node에서
  nethook clean run은 `egressAttempts=0`, blocked-control run은 `dns.lookup example.com` 관측·차단(`egressAttempts=1`).
  ⑤ **검증**: `git diff --check` PASS(LF/CRLF warning only), bootstrap 12/12, Node runner 39/39, Python runner 49/49,
  nethook 29/29. repo contamination scan에서 runtime zip/binary/node_modules/package-lock/submission.zip/generated artifacts 미커밋.
- **Consequences**: Portable Node B의 공식 다운로드·무결성·repo 밖 배치 경로는 evidence 수준에서 확인됨. 다음 분기는
  ChatGPT/user 판단(예: 별도 review 여부, 2N-5 환경 정의, B안 채택 여부)으로 남김.
- **Status**: 변경 = evidence 문서, ps1 hash constant, runner README, 관련 Node tests, current_status/decision_log. core/ingest/schema/
  validator/renderer/delivery/Skill/manifest/marketplace/package-lock 무변경. 실제 `npm install`/Kordoc reinstall/OCR/2N-5 미수행.
- **Related Files**: `docs/samples/codex_cycle2n_4g_portable_node_real_download_evidence.md`,
  `src/intake/runners/prepare_portable_node.ps1`, `src/intake/runners/README.md`,
  `tests/test_portable_node_bootstrap.test.cjs`, `tests/test_hwp_assisted_runner_node.test.cjs`.

## D90. Portable Node B안 채택 결정 — Windows Node 부재 환경의 승인 기반 fallback runtime strategy
- **Date**: 2026-07-06
- **Context**: 2N-4E 계획(PASS)·2N-4F mock(CONDITIONAL→2N-4F-A 보정 PASS)·**2N-4G official real-download evidence(D89)** +
  **Claude 2N-4G evidence review PASS**(hash 5중 일치·로컬 실물 corroboration·Critical/Major 0)를 근거로,
  사용자 결정에 따라 B안을 **채택**한다(2N-4H-UX에서 반영).
- **Decision(채택 — 범위 한정)**:
  B안 = **"시스템 Node/npm이 없는 Windows 사용자 환경에서, 명시적 승인 후 공식 nodejs.org portable Node를
  repo 밖 tool-cache에 준비하는 fallback runtime strategy"**.
  ① 공식 source 고정: `https://nodejs.org/dist/v24.16.0` (미러/임의 URL은 네트워크 호출 전 거부 — 2N-4F-A).
  ② 무결성: **repo-pinned SHA-256(`edaca9bd…cd56`) + SHASUMS256.txt 교차검증** — 어느 한쪽 불일치도 fail-fast·정리.
  ③ 배치: repo 밖 tool-cache(`node@v24.16.0-win-x64/`) — OS installer/PATH 영구 수정/관리자 권한 없음(C안 배제 불변),
  제거=폴더 삭제. ④ 탐지 우선순위: 시스템 Node/npm 우선 → portable(버전 실측 일치 필수 — 2N-4F-A) → 승인 안내.
  ⑤ **거부·준비 실패는 전부 A안(기본 텍스트 기반 검토 계속) 수렴** — 무단 설치 없음.
  ⑥ 실행 단계 **no-egress 경계 유지**(nethook — portable node에서 관측 evidence 확보, 2N-4G).
- **채택이 의미하지 않는 것(명시)**: **B안 채택 완료 ≠ 2N-5 unblock ≠ 플러그인 완성 ≠ L2 complete ≠
  OCR support complete ≠ provider finalization.** B안은 core plugin의 필수 dependency가 아니라 선택적 assisted
  경로의 런타임 fallback이다. portable `npm.cmd`로 실제 Kordoc 설치 성립 확인은 **선택적 잔여 evidence**
  (C2N4E-OBS-03 후반)일 수 있으나 **채택의 차단 조건이 아니다**(Claude 리뷰 판단 — 시스템 npm.cmd와 동일 배포물).
- **Status**: 결정 기록 + 사용자-facing 문서 반영(2N-4H-UX — quickstart/README/runner README). 코드 무변경.
- **Related Files**: `docs/samples/codex_cycle2n_4g_portable_node_real_download_evidence.md`,
