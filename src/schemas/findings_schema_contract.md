...
> **번들 자기완결 사본**: 이 파일은 **설치 플러그인 번들(루트 = 저장소 `src/`)** 자기완결성을 위해 스킬 계약을
> 번들 안에 둔 것이다. 개발 기준 원본은 저장소 루트 `docs/findings_schema_contract.md`이며, 두 사본은 동일 계약을
> 담는다(동기화 책임 명시 — 계약 변경 시 두 파일을 함께 갱신). **경로 규약**: 이 문서의 경로는 **설치 플러그인 루트
> 기준**이다(예: `schemas/kssb_findings.schema.json`). 저장소 개발 트리에서는 동일 파일이 `src/` 접두로 존재한다.
...
## 파일 위치
- 스키마: `schemas/kssb_findings.schema.json` (JSON Schema draft-07, 외부 런타임 의존 없음)
- 예시: `schemas/kssb_findings_example.json` (가상 공개 보고서 입력, 실제 기업·파일명 미사용)
...
- 이 매핑은 `skills/samil-kssb-precheck/judgment_schema.md`의 라벨 세트와 일치한다.
...
렌더러가 `skills/samil-kssb-precheck/report_template.md`의 질문 열(질문사유·관련근거·우선순위·요청자료·후속조치)을 안정적으로 렌더할 수 있도록,
