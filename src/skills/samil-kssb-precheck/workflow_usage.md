...
> **번들 자기완결 사본**: 이 파일은 **설치 플러그인 번들(루트 = 저장소 `src/`)** 자기완결성을 위해 워크플로우/전달
> 계약을 번들 안에 둔 것이다. 개발 기준 원본은 저장소 루트 `docs/workflow_usage.md`이며, 두 사본은 동일 계약을
> 담는다(동기화 책임 명시). **경로 규약**: 이 문서에서 `src/…` 접두 경로는 **저장소 개발 트리 기준** 표기이며,
> **설치 플러그인 루트 기준에서는 `src/` 접두를 제거해 읽는다**(예: `renderers/kssb_report_delivery.cjs`,
> `validators/kssb_findings_validator.cjs`). `docs/blackbox_protocol.md`·`docs/submission_packaging_policy.md`는
> **개발/검증·제출 패키징 문서(저장소 전용)**이며 설치 스킬 실행에 필수가 아니다.
...
Skill (판단 엔진)
  └─ 입력 자료(고객 제공자료 또는 해커톤 공개자료)를 근거로 KSSB 4대 영역 항목별
     판정·근거 앵커·부족정보·고객 질문·권고를 source-bound 구조화 findings로 생성
        → schemas/kssb_findings.schema.json / schemas/findings_schema_contract.md 계약 준수
Validator (detect-only preflight 게이트)
  └─ findings를 재판정 없이 점검(구조 필수 필드·source_id cross-reference·모드↔라벨 정합·
     ...
     런타임 = Node(validators/kssb_findings_validator.cjs), Python(.py)은 golden parity reference.
...
Renderer / Delivery (형식 변환기 + 전달 배선)
  └─ 동일 findings를 재판정 없이 대표 문서(DOCX → HTML → Markdown)로 결정적 변환(단일 소스 파생).
     런타임 = Node(renderers/kssb_report_delivery.cjs → renderers/kssb_report_renderer.cjs, D94 hard stop 내장),
...
런타임 구현은 Node `renderers/kssb_report_delivery.cjs`, Python `renderers/kssb_report_delivery.py`는
...
- **preflight error hard stop 정책(D94 — Node delivery에 구현됨, 2N-6 Phase 2 N2)**: preflight에서
  ...
  **구현 위치는 Node delivery(`renderers/kssb_report_delivery.cjs`)**이며(2N-6 Phase 2 N4에서 Node
  ...
  판정 기준(`docs/blackbox_protocol.md` §3-(a): preflight error 0이어야 PASS — 개발·검증 프로토콜, 저장소 전용)이 이 구간을 커버한다.

## 내부/검증용 실행 (개발·CI 참고, 사용자 흐름 아님)

> 아래 명령의 `src/…` 경로는 **저장소 개발 트리 기준**이다(설치 플러그인 루트에서는 `src/` 접두 제거).
> 이 절은 개발·CI 참고용이며 설치 스킬의 기본 사용자 흐름이 아니다.
...
- **trace manifest(`run_manifest.json`)**: Node delivery의 **opt-in(기본 off)** 부가 산출물이다. `--manifest`/
  ...
  아니므로 `.gitignore`로 repo 유입을 방어한다(`docs/blackbox_protocol.md` §4의 수동 provenance 집계를 대체 가능 — 저장소 전용 문서).
- 생성 문서는 커밋 대상이 아니며 findings에서 결정적으로 재생성 가능하다(`.gitignore` 산출물 제외).
- plugin/cache/sandbox 내부 경로를 산출물·사용자 안내에 노출하지 않는다.
- 제출 패키징(포함/제외 분류·원본 로그 방식·샘플 산출물 위치·최종 preflight)은 `docs/submission_packaging_policy.md` 참조(제출 패키징 문서, 저장소 전용).
...
- **core 워크플로우는 문서 변환·OCR을 자동 실행하지 않는다.** 문서 구조 판독과 스캔/혼합 PDF의 문자 인식(OCR)은
  core 밖 **승인 기반 로컬 보조 runner**(`intake/runners/` — 자동 실행 없음, 최소 경로)로만 제공되며,
