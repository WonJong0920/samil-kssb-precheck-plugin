...
- 수치 표 행을 인용할 때는 근거 설명에 **열↔기간(연도)·단위 매핑**을 명시하고, 같은 숫자가 다른 의미(목표 수준 vs 감축량 등)로
  읽힐 수 있으면 근거 설명에서 구분하거나 고객 확인 질문으로 표면화한다(`evidence_mapping_rules.md` §7 — 서술·질문 층 규칙, 재판정 아님).
